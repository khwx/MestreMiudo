...
- **Tarefa implementada:** Adicionadas 5 novas perguntas de Estudo do Meio em `src/lib/questions/estudo-do-meio.ts` (Fase 5 complementar):
  - **Eletricidade G4 (+1):** circuito fechado necessário para corrente.
  - **Fotossíntese G3 (+1):** ocorre nas folhas (cloroplastos).
  - **Ciência G4 (+2):** variável experimental, grupo de controlo.
  - **Habitats G4 (+1):** definição de habitat.
  - IDs `em-exp5-011` a `em-exp5-015`.
- **Validação:** Banco passou de 881 a 886 perguntas; `npm run lint` ✓, `npm run typecheck` ✓, suíte completa: 544 testes a passar.
- **Docs atualizados:** registo desta expansão.

## 2026-09-06 - Expansão banco de perguntas Matemática (+10 questões G1/G2/G3/G4)

- **Contexto:** Continuação da expansão do banco em tópicos com pouca cobertura (análise de `TODO.md` e cobertura). Foco em Matemática onde persistem tópicos com 1 pergunta.
- **Tarefa implementada:** Adicionadas 10 novas perguntas de Matemática em `src/lib/questions/matematica.ts` (Fase 5):
  - **Gráficos Simples G1 (+1):** leitura de barras.
  - **Problemas com Multiplicação G2 (+1):** multiplicação contextualizada.
  - **Estimativas G2 (+1):** arredondamento para soma.
  - **Multiplicação G2/G3 (+2):** tabuada (4×6=24, 8×7=56).
  - **Divisão G2 (+1):** partilha igual.
  - **Frações G2 (+1):** 1/2 = metade.
  - **Resolução 2 Passos G3/G4 (+2):** subtrações sucessivas.
  - **Números Decimais G3 (+1):** 3/4 = 0,75.
  - IDs `mat-exp5-001` a `mat-exp5-010`.
- **Validação:** Banco passou de 871 a 881 perguntas; `npm run lint` ✓, `npm run typecheck` ✓, suíte completa: 544 testes a passar.
- **Docs atualizados:** registo desta expansão.
...
- **Tarefa implementada:** Adicionadas 8 novas perguntas de Português em `src/lib/questions/portugues.ts` (Fase 7):
  - **Gramática G2 (+2):** conceito de palavra, pontuação.
  - **Ortografia G2 (+2):** acentuação aguda (pássaro, café).
  - **Género G2 (+2):** masculino/feminino (menino/menina).
  - **Plural G2 (+2):** regras -s e -éis (gato→gatos, papel→papéis).
  - IDs `pt-exp7-001` a `pt-exp7-008`.
- **Validação:** Banco passou de 850 a 858 perguntas; `npm run lint` ✓, `npm run typecheck` ✓, suíte completa: 544 testes a passar.
- **Docs atualizados:** registo desta expansão.

## 2026-09-06 - Expansão banco de perguntas Português (+24 questões total)

- **Contexto:** Continuação da expansão do banco em tópicos com pouca cobertura (análise de `TODO.md` e cobertura). Foco em Português G3/G4 onde persistem tópicos com 1 pergunta.
- **Tarefa implementada:** Adicionadas 24 novas perguntas de Português em `src/lib/questions/portugues.ts` (quatro fases):
  ...
  - **Fase 3 (12):** Biografia G3 (+2), Notícia G3 (+2), Argumentação G3/G4 (+4), Texto Descritivo G4 (+2), Entrevista G4 (+2). IDs `pt-exp6-001` a `pt-exp6-012`.
- **Validação:** Banco passou de 826 a 862 perguntas; `npm run lint` ✓, `npm run typecheck` ✓, suíte completa: 544 testes a passar.
- **Docs atualizados:** registo desta expansão.
...
- **Tarefa implementada:** Adicionadas 6 novas perguntas de Português em `src/lib/questions/portugues.ts`:
  - **Texto Descritivo G1/G2 (+4):** caracterização de objetos/animais, uso de adjetivos.
  - **Identificação de Personagens G1 (+1):** conceito de personagens.
  - **Sequência de Eventos G1 (+1):** ordenação inicial do dia.
  - IDs `pt-exp5-001` a `pt-exp5-006`.
- **Validação:** Banco passou de 826 a 832 perguntas; `npm run lint` ✓, `npm run typecheck` ✓, suíte completa: 544 testes a passar.
- **Docs atualizados:** registo desta expansão.
...
- **Tarefa implementada:** Guardar o recorde de pontuação por ano letivo em `localStorage`:
  1. `src/components/MagicSequenceGame.tsx` — novos estados `bestScore` (inicializado a partir de `localStorage` com chave `magic-sequence-best-${grade}`) e `isNewBestScore`; ref `scoreRef` para ler a pontuação final sem closures obsoletos.
  2. `endGame` — no fim do jogo, se a pontuação for maior que o recorde (ou primeira), guarda e marca `isNewBestScore`.
  3. Ecrã de resultados — mostra "🎉 Novo Recorde!" quando bate o recorde, senão "Recorde: X".
  4. `startGame` carrega o recorde ao escolher o ano letivo.
- **Validação:** `npm run lint` ✓, `npm run typecheck` ✓, suíte completa: 544 testes a passar (57 ficheiros).
- **Docs atualizados:** registo desta melhoria.

## 2026-09-06 - Melhor tempo (recorde) nas Palavras Cruzadas

- **Contexto:** As Palavras Cruzadas (CrosswordGame) cronometravam o jogo (`time`) mas nunca guardavam o melhor tempo, ao contrário da Sopa de Letras e do Jogo da Memória que já tinham recorde local. Não havia forma de a criança ver o seu melhor desempenho nem competir consigo própria por disciplina.
- **Tarefa implementada:** Guardar o melhor tempo por disciplina em `localStorage`:
  1. `src/components/CrosswordGame.tsx` — novo estado `bestTime` (inicializado a partir de `localStorage` com chave `crossword-best-${subject}`) e `isNewRecord`; ref `timeRef` para ler o tempo atual sem closures obsoletos no `setInterval`.
  2. Ao completar o cruzograma (`checkWin`), se o tempo for menor que o recorde (ou primeiro), guarda e marca `isNewRecord`.
